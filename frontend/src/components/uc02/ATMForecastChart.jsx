...
import { runATMForecast } from '../../hooks/useAPI'
import { formatYAxis } from '../../utils/formatPKR'

export default function ATMForecastChart() {
  const {
    selectedATM,
    atmForecastData,
    setATMForecastData,
  ...
  const handleRunForecast = async () => {
    ...
    setError(null)
    try {
      const { data } = await runATMForecast(selectedATM.atm_id)
      // Transform separate arrays into chart-ready format (same as UC-01 fix)
      let chartData = data.forecast_data || data.data || []
      if (chartData.length === 0 && data.forecast_dates) {
        const hist = (data.historical || []).map(h => ({
          date: h.date, actual: h.actual, forecast: h.predicted,
        }))
        const forecast = data.forecast_dates.map((d, i) => ({
          date: d,
          forecast: data.predicted_dispense?.[i] || data.predicted_demand?.[i],
          confidence_upper: data.confidence_upper?.[i],
          confidence_lower: data.confidence_lower?.[i],
        }))
        chartData = [...hist, ...forecast]
      }
      setATMForecastData(chartData)
      setMetrics(data.model_metrics || data.metrics || null)
    } catch (err) {
      setError('Failed to run ATM forecast. Check backend connection.')
      console.error('ATM Forecast error:', err)
    } finally {
      setLoading(false)
    }
  }

  const forecastData = atmForecastData || []
      ...
      {forecastData.length === 0 ? (
        ...
        >
          <p className="text-sm" style={{ color: '#6b7280' }}>
            {selectedATM ? 'Click RUN FORECAST to generate prediction' : 'Select an ATM first'}
          </p>
        </div>
      ) : (
        <>
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={forecastData} margin={{ top: 5, right: 20, bottom: 5, left: 20 }}>
              ...
              {/* Capacity reference */}
              {selectedATM?.total_capacity && (
                <ReferenceLine
                  y={selectedATM.total_capacity}
                  stroke="#d4a853"
                  strokeDasharray="8 4"
                  strokeWidth={1.5}
                  label={{
