...
The app already has 29 pages. Add this as page #30.
...
In your sidebar/nav component, add at the TOP — above Executive Summary.
This is the primary demo page for executives:
```jsx
import { Zap } from 'lucide-react'
// Add nav item at the TOP of sidebar:
...
### HOW COMMAND CENTER CONNECTS TO EXISTING PAGES

The Command Center does NOT duplicate your existing pages. It LINKS to them:

- KPI "Total Idle Cash" card → links to /vault-heatmap (VaultHeatmap.jsx)
- KPI "Annual KIBOR Loss" → links to /pnl-waterfall (PnLWaterfall.jsx)
- KPI "Avg CES" → links to /branch-scorecard (BranchScorecard.jsx)
- Branch scatter dots → click navigates to /branch-plan?id=KHI-0001 (BranchPlanView.jsx)
- "View full netting analysis" link → /uc/03 (UC03Dashboard.jsx)
- "View CRR position" link → /crr-gauge (CRRGauge.jsx)

The Command Center is the ONLY page with Execute buttons and live state mutation.
All other pages are read-only views. This is the action page.

### WHAT MAKES THIS DIFFERENT FROM EXISTING PAGES

Your existing pages:
  - /consolidated → shows KPIs (READ ONLY)
  - /regional → shows netting matches (READ ONLY)
  - /api/business/kdtree-netting → finds opportunities (READ ONLY)

Command Center:
  - Shows opportunities PLUS "⚡ Execute" buttons
  - Clicking Execute MUTATES vault balances in memory
  - KPIs UPDATE LIVE after each click (idle cash drops, CES rises)
  - Success toast shows BEFORE → AFTER vault states
  - Cumulative P&L delta stacks with each transfer
  - Reset button restores baseline
  - THIS IS THE DEMO — everything else is the dashboard

Think of it as: existing pages = Bloomberg terminal (watching data)
Command Center = trading desk (taking action and seeing the effect)
